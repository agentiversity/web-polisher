...
1. `manifest.json` with basic permissions
2. Empty background service worker
3. Empty content script
4. Test: Extension loads, scripts inject

**Phase 2: Content Detection (Week 2)**
1. Content detector with site-specific selectors (Facebook, Reddit)
2. Heuristic fallback for generic sites
3. Test: Correctly identifies comments vs UI on target sites

**Phase 3: LLM Integration (Week 3)**
1. Background worker with LLM API client (Gemini Flash)
2. Message passing: content script → background → content script
3. Basic polish prompt
4. Test: Text sent to LLM, response received

**Phase 4: DOM Replacement (Week 4)**
1. Safe text replacement logic
2. Preserve original HTML for debugging
3. Test: Text replaced without breaking page

**Phase 5: Lazy Loading (Week 5)**
1. IntersectionObserver integration
2. Process only visible content
3. Test: Comments polished as scrolled into view

**Phase 6: Polish (Week 6)**
1. Confidence threshold logic
2. Caching (avoid re-polishing same text)
3. Error handling (API failures, rate limits)
4. Test: Low-confidence transformations aborted
