...
**Phase:** Phase 1 (core text replacement) — must get this right before anything else.
...
**Phase:** Phase 1 (lazy loading requirement) — core to acceptable user experience.
...
**Phase:** Phase 1 (content detection) — fundamental to not breaking sites.
...
- **Test CORS early.** In Phase 1, verify API calls work from background script before building content script logic.
...
**Phase:** Phase 1 (LLM integration) — architectural decision, must get right before building features.
...
**Phase:** Phase 2 (quality tuning) — after core functionality works, refine threshold.
...
**Phase:** Phase 2 (performance optimization) — after MVP works, add cache management.
...
**Phase:** Phase 2 (lazy loading refinement) — after basic lazy loading works, handle edge cases.
...
**Phase:** Phase 1 (LLM integration) — must decide architecture early.
...
**Phase:** Phase 2 (error handling) — after core flow works.
...
**Phase:** Phase 1 (lazy loading) — simple fix, do it right first time.
...
**Phase:** Phase 1 (Firefox-specific setup) — handle early to avoid confusion.
...
|-------------|---------------|------------|
| **Phase 1: Core text replacement** | Breaking React/Vue state (#1) | Use TreeWalker on text nodes only, test on Facebook/Reddit early |
| **Phase 1: Content detection** | Polishing UI elements (#3) | Site-specific selectors, exclude buttons/links/labels |
| **Phase 1: LLM integration** | CORS restrictions (#4) | Route API calls through background script |
| **Phase 1: LLM integration** | API key exposure (#8) | User-provided keys in settings, no hardcoded secrets |
| **Phase 1: Lazy loading** | Janky scroll (#2) | IntersectionObserver with rootMargin, debounce scroll events |
| **Phase 1: Lazy loading** | Duplicate polishing (#10) | Mark polished elements, cache by content hash |
| **Phase 1: Firefox setup** | Navigation lifecycle (#11) | Use `browser.storage.session`, listen to pageshow/pagehide |
| **Phase 2: Quality tuning** | Confidence threshold too loose (#5) | Semantic similarity check, length check, tone preservation in prompt |
| **Phase 2: Performance** | Memory leaks (#6) | LRU cache with size limit, TTL eviction |
| **Phase 2: Lazy loading refinement** | Race conditions (#7) | Verify text before updating, use WeakRef, re-observe on re-add |
| **Phase 2: Error handling** | Offline mode (#9) | Detect offline, fail silently, skip polishing |
