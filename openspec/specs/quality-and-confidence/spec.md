...
- **WHEN** a user adjusts the confidence threshold in settings
- **THEN** the extension applies the new threshold to subsequent transformations, with a conservative default
...
- **THEN** the extension evaluates the output (e.g. semantic similarity to the original and length fidelity) before replacing any page text

#### Scenario: Failed quality gate shows original
- **WHEN** a polished result fails the quality gate
