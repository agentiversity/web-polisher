...
import { isApplyPolishMessage, isPolisherStatusMessage, isPolisherStatusReply, type ApplyPolishMessage } from '../../utils/messages';
...
export async function initPopup(doc: Document): Promise<void> {
  ...
  try {
    ...
    if (tab?.id != null) {
      ...
      if (isPolisherStatusReply(reply)) setStatus(reply.status);
    }
  } catch {
  ...
  browser.runtime.onMessage.addListener((message: unknown) => {
    if (isPolisherStatusMessage(message)) setStatus(message.status);
  });
